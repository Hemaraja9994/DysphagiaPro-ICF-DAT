...
function FormField({ label, type = 'text', value, onChange, options, placeholder, required, className, rows }: {
  ...
  if (type === 'select') return (
    <div className={cn("mb-3", className)}>
      <label className="block text-sm font-medium text-foreground/80 mb-1">{label}{required && <span className="text-destructive ml-0.5">*</span>}</label>
  ...
  if (type === 'textarea') return (
    <div className={cn("mb-3", className)}>
      ...
      <textarea value={value || ''} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows || 3}
        className="w-full border border-input rounded-md px-3 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y" />
    </div>
  );
  if (type === 'radio') return (
    <div className={cn("mb-3", className)}>
      ...
      <div className="flex flex-wrap gap-2">
        {options?.map(o => { const v = typeof o === 'string' ? o : o.value; const l = typeof o === 'string' ? o : o.label;
          return (
            <label key={v} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm cursor-pointer transition-all",
    ...
    <div className={cn("mb-3", className)}>
      ...
      <div className="flex flex-wrap gap-2">
        {options?.map(o => { const v = typeof o === 'string' ? o : o.value; const l = typeof o === 'string' ? o : o.label;
          ...
          return (
            <label key={v} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm cursor-pointer transition-all",
              checked ? "bg-primary/10 text-primary border-primary/40" : "bg-background border-input hover:bg-accent")}>
...
function Card({ title, children, color, className }: { title?: string; children: React.ReactNode; color?: string; className?: string }) {
  return (
    <div className={cn("bg-card rounded-xl border shadow-sm p-5 mb-4", className)}>
      {title && <h3 className="text-base font-bold mb-4 pb-2 border-b flex items-center gap-2" style={{ color: color || 'var(--foreground)' }}>
        {title}
      </h3>}
      {children}
    </div>
...
function ScoreButton({ score, selected, onClick, color }: { score: number | string; selected: boolean; onClick: () => void; color?: string }) {
  return (
    <button type="button" onClick={onClick}
      className={cn("w-9 h-9 rounded-lg border-2 flex items-center justify-center text-xs font-bold transition-all",
        selected ? "text-white shadow-sm" : "bg-background hover:bg-accent")}
      style={selected ? { backgroundColor: color || '#3b82f6', borderColor: color || '#3b82f6' } : { borderColor: '#e5e7eb' }}>
      {score}
    </button>
  );
}
      ...
      <div className="flex gap-1 flex-shrink-0">
        {options.map(o => (
          <button key={o.value} type="button" onClick={() => onChange(o.value)}
            className={cn("px-2 py-1 text-xs rounded-md border font-medium transition-all",
              value === o.value ? "text-white" : "bg-background hover:bg-accent")}
            style={value === o.value ? { backgroundColor: o.color, borderColor: o.color } : {}}>
            {o.label}
          </button>
        ))}
      </div>
...
function ICFDATModule({ data, set }: { data: any; set: (d: any) => void }) {
    ...
    return (
      <div className="border rounded-lg mb-2 overflow-hidden">
        <div className="flex items-center gap-2 p-3 bg-muted/30">
          <span className="bg-primary/10 text-primary px-2 py-0.5 rounded text-xs font-mono font-bold flex-shrink-0">{item.code}</span>
          <span className="flex-1 text-sm font-medium cursor-pointer" onClick={() => setOpen(!open)}>{item.name} <span className="text-muted-foreground text-xs">{open ? '▲' : '▼'}</span></span>
          <div className="flex gap-1 flex-shrink-0">
        ...
        {open && <div className="p-3 bg-background border-t text-xs text-muted-foreground">
          <p><strong>Description:</strong> {item.description}</p>
          {item.inclusions && <p><strong>Inclusions:</strong> {item.inclusions}</p>}
          {item.exclusions && <p><strong>Exclusions:</strong> {item.exclusions}</p>}
          <input type="text" placeholder="Description of the problem..." value={icf.notes?.[item.code] || ''} className="w-full mt-2 text-xs border rounded px-2 py-1"
  ...
  return (<>
    <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-5 rounded-xl mb-4 shadow-lg">
      <h3 className="text-xl font-bold">WHO-ICF Based Dysphagia Assessment Tool</h3>
      <p className="text-blue-200 text-sm mt-1">Copyright Reg. No. LD-20250171253 | Hemaraja Nayaka S, Dr. Vijayalakshmi S, Dr. Jayashree Bhat</p>
      <div className="mt-3 bg-white/10 rounded-lg p-3">
        <div className="flex justify-between text-sm mb-1"><span>Progress</span><span className="font-bold">{rated}/{total} items</span></div>
        <div className="h-2 bg-white/20 rounded-full overflow-hidden"><div className="h-full bg-white/80 rounded-full transition-all" style={{width:`${(rated/total)*100}%`}} /></div>
      </div>
    </div>
    ...
    {/* Qualifier Legend */}
    <div className="bg-card rounded-xl border p-3 mb-4 flex flex-wrap gap-3">
      {ICF_QUALIFIERS.map(q => (
        <div key={q.value} className="flex items-center gap-1"><div className="w-5 h-5 rounded text-white text-xs font-bold flex items-center justify-center" style={{backgroundColor:q.color}}>{q.value}</div>
          <span className="text-xs text-muted-foreground">{q.label}</span></div>
      ))}
    </div>
    {/* Section Nav */}
    <div className="flex flex-wrap gap-2 mb-4">
      {sections.map(s => (
        <button key={s.id} type="button" onClick={() => setSection(s.id)}
          className={cn("px-4 py-2 rounded-lg text-sm font-medium transition-all", section===s.id ? "text-white shadow" : "bg-card border hover:bg-accent")}
          style={section===s.id ? {backgroundColor: s.color} : {}}>
          {s.label} <span className="ml-1 text-xs opacity-80">({s.count})</span>
        </button>
      ))}
    </div>
    {/* Content */}
    ...
    {section === 'ap' && <Card title="Section III: Activities & Participation">
      <p className="text-sm text-muted-foreground mb-3"><strong>Performance:</strong> actual in current environment | <strong>Capacity:</strong> ability without assistance</p>
      {ACTIVITIES_PARTICIPATION.map(item => (
        <div key={item.code} className="border rounded-lg mb-2 p-3 bg-muted/20">
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded text-xs font-mono font-bold">{item.code}</span>
            <span className="text-sm font-medium">{item.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
            <div><span className="text-xs font-bold text-blue-600 mr-2">Performance:</span>
            ...
            <div><span className="text-xs font-bold text-purple-600 mr-2">Capacity:</span>
    ...
    {section === 'ef' && <Card title="Section IV: Environmental Factors">
      <p className="text-sm text-muted-foreground mb-3">Rate as <strong className="text-destructive">Barriers (0-4)</strong> and <strong className="text-emerald-600">Facilitators (+1 to +4)</strong></p>
      {ENVIRONMENTAL_FACTORS.map(item => (
        <div key={item.code} className="border rounded-lg mb-2 p-3 bg-muted/20">
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded text-xs font-mono font-bold">{item.code}</span>
            <span className="text-sm font-medium">{item.name}</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
            <div><span className="text-xs font-bold text-destructive mr-2">Barrier:</span>
            ...
            <div><span className="text-xs font-bold text-emerald-600 mr-2">Facilitator:</span>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="bg-gradient-to-r from-blue-700 to-indigo-800 text-white shadow-lg sticky top-0 z-50 no-print">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden p-1"><Menu size={22} /></button>
            <div>
              <h1 className="text-lg font-bold tracking-tight">DysphagiaPro</h1>
              <p className="text-blue-200 text-xs">WHO-ICF Assessment Tool</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSave} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all", saved ? "bg-emerald-500" : "bg-white/10 hover:bg-white/20")}>
              {saved ? <CheckCircle2 size={16} /> : <Save size={16} />} {saved ? 'Saved!' : 'Save'}
            </button>
            <button onClick={handleNew} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-white/10 hover:bg-white/20">
      ...
      <div className="max-w-7xl mx-auto flex">
        ...
        <AnimatePresence>
          {(sidebarOpen || typeof window !== 'undefined') && (
            <aside className={cn("fixed lg:sticky top-[60px] left-0 h-[calc(100vh-60px)] w-64 bg-card border-r z-40 overflow-y-auto no-print transition-transform lg:translate-x-0",
              ...
              <div className="p-3">
                ...
                {data.demographics?.name && (
                  <div className="bg-primary/5 rounded-lg p-3 mb-3 border border-primary/20">
                    <div className="text-sm font-bold text-primary">{data.demographics.name}</div>
                    <div className="text-xs text-muted-foreground">{data.demographics.age && `${data.demographics.age}y`} {data.demographics.gender} {data.demographics.hospitalNumber && `| ${data.demographics.hospitalNumber}`}</div>
                  </div>
                )}
                {/* Tab Navigation */}
                  ...
                  {TABS.map((tab, i) => (
                    <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSidebarOpen(false); }}
                      className={cn("w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-all text-left",
                        activeTab === tab.id ? "bg-primary/10 text-primary font-medium shadow-sm" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>
                      <span className="flex-shrink-0" style={{color: activeTab === tab.id ? tab.color : undefined}}>{tab.icon}</span>
                ...
                {assessments.length > 0 && (
                  <div className="mt-6 pt-4 border-t">
                    <h4 className="text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wider">Saved Assessments</h4>
                    {assessments.slice(-5).reverse().map(a => (
                      <div key={a.id} className={cn("flex items-center justify-between p-2 rounded-lg text-xs mb-1 cursor-pointer",
                        a.id === data.id ? "bg-primary/10" : "hover:bg-muted")} onClick={() => handleLoad(a)}>
                        <div><div className="font-medium">{a.demographics?.name || 'Unnamed'}</div>
                          <div className="text-muted-foreground">{a.updatedAt ? new Date(a.updatedAt).toLocaleDateString() : ''}</div></div>
                        <button onClick={e => { e.stopPropagation(); handleDelete(a.id); }} className="text-muted-foreground hover:text-destructive"><Trash2 size={14} /></button>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <main className="flex-1 p-4 lg:p-6 min-h-[calc(100vh-60px)]">
          ...
          <div className="flex items-center gap-2 mb-4 text-sm text-muted-foreground no-print">
            ...
            <span className="font-medium text-foreground">{TABS[tabIdx]?.label}</span>
            <span className="ml-auto text-xs">Step {tabIdx + 1} of {TABS.length}</span>
          </div>
          ...
          <div className="flex justify-between mt-6 no-print">
            <button onClick={goPrev} disabled={tabIdx === 0}
              className={cn("flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-all", tabIdx === 0 ? "bg-muted text-muted-foreground" : "bg-card border hover:bg-accent shadow-sm")}>
              <ChevronLeft size={18} /> Previous
            </button>
            <button onClick={goNext} disabled={tabIdx === TABS.length - 1}
              className={cn("flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-all", tabIdx === TABS.length - 1 ? "bg-muted text-muted-foreground" : "bg-primary text-primary-foreground hover:opacity-90 shadow-sm")}>
              Next <ChevronRight size={18} />
            </button>
          </div>

          {/* Footer */}
          <footer className="mt-8 py-4 border-t text-center text-xs text-muted-foreground no-print">
            <p className="font-medium">DysphagiaPro — WHO-ICF Based Dysphagia Assessment Tool (ICF-DAT)</p>
            <p>Copyright © 2025 Hemaraja Nayaka S, Dr. Vijayalakshmi Subramaniam, Dr. Jayashree Bhat</p>
